...
(function () {
    ...
    if (!internalFolder.exists) {
        throw new Error("Break Text into Words internal folder missing: " + internalFolder.fsName);
    }

    TextBreak = {
        moduleId: "break_text_into_words",
        granularity: "words",
        title: "Break Text into Words"
    };

    $.global.__TOOLKIT_BREAK_TEXT_WORDS__ = TextBreak;
    ...
    for (index = 0; index < internalFiles.length; index += 1) {
        ...
        if (!file.exists) {
            throw new Error("Break Text into Words internal file missing: " + file.fsName);
        }
        $.evalFile(file);
    }

    if (typeof TextBreak.execute !== "function") {
        throw new Error("Break Text into Words module failed to register execute().");
    }
