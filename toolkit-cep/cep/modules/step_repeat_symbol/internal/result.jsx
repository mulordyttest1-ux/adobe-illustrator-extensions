...
StepRepeat.buildSuccessMessage = function (outcome) {
    var message = "Created " + outcome.count + " symbol instance(s) on active artboard " + (outcome.activeArtboardIndex + 1) + ".";
...
StepRepeat.execute = function (payload) {
    ...
    if (!app.documents.length) {
        return StepRepeat.createFailure(
            "Open a document before running Step Repeat Symbol.",
    ...
    try {
        ...
        return StepRepeat.createFailure(
            error && error.message ? error.message : "Step Repeat Symbol failed.",
            "STEP_REPEAT_FAILED"
        );
    }
};
