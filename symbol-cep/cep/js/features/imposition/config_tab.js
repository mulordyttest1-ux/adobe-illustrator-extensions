 ...
 * DEPENDENCIES: ConfigRenderer, ConfigEvents, ConfigPersistence, DataStore
 * SIDE EFFECTS: DOM manipulation
 * EXPORTS: window.Imposition.configTab
 */
    ...
    class ConfigTab {
        ...
        init(containerId) {
            ...
            if (lastId) {
                ...
                window.Imposition.ConfigPersistence.loadPreset(lastId, this);
                const sel = document.getElementById('load-preset-select');
                if (sel) sel.value = lastId;
            }

            window.Imposition.ConfigEvents.bindEvents(this);
        }
        ...
        render() {
            ...
            const R = window.Imposition.ConfigRenderer;
            ...
            this.container.innerHTML = `
                <form id="config-form">
                    ...
                    <div id="dynamic-form-body">
                        ${schema ? R.renderSchema(schema, this.isEditMode) : '<p>Error: Schema not loaded.</p>'}
                    </div>
                    ${this._renderFooter()}
                    ${this._renderModal()}
                </form>
