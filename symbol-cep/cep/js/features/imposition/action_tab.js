 ...
 * DEPENDENCIES: DataStore, BuiltinPresets, ConfigEngine
 * SIDE EFFECTS: DOM, evalScript (CEP)
 * EXPORTS: ActionTab class, actionTab singleton
 */
...
export class ActionTab {
    ...
    init(containerId) {
        ...
        this.container.addEventListener('click', (e) => {
            const triggerItem = e.target.closest('.dropdown-item');
            const triggerBtnManager = e.target.closest('button.manager-run-btn');
        ...
        this.container.addEventListener('keydown', (e) => {
            if (e.target.id === 'action-search' && !this.isManagerMode) {
                if (e.key === 'ArrowDown') {
                    ...
                    if (selected) { this.handleTrigger(selected.id); }
                }
            }
        });
    ...
    _renderDropdownContent(container) {
        if (this.filteredPresets.length === 0) {
            container.innerHTML = `<p style="text-align: center; color: #666; font-size: 12px; margin-top: 20px;">Không tìm thấy cấu hình phù hợp.</p>`;
            return;
        }
    ...
    _renderManagerList(container, presets) {
        ...
        if (list.length === 0) {
            container.innerHTML = `<p style="text-align: center; color: #666; font-size: 12px; margin-top: 20px;">${this.searchTerm ? 'Không tìm thấy preset nào.' : 'Chưa có cấu hình. Hãy tạo mới bên Tab Config.'}</p>`;
            return;
        }
    ...
    _compileRules(preset) {
        if (!ConfigEngine || !BuiltinPresets || !preset.rawValues) return;
        try {
            console.log("-> Compiling Rules...");
            const schemaId = preset.schemaId || 'standard_imposition';
            const schema = BuiltinPresets.find(function (s) { return s.id === schemaId; });
            if (!schema) return;

            const rules = ConfigEngine.compileRules(schema, preset.rawValues);
            if (rules.length > 0) {
                preset.rules = rules;
    ...
    _runImpositionEngineAsync(preset) {
        return new Promise((resolve) => {
            // DTO via Base64 - Safe UTF-8 encoding
            const payloadStr = JSON.stringify(preset);
            const payloadBase64 = window.btoa(unescape(encodeURIComponent(payloadStr)));

            console.log(`🚀 Dispatching [${preset.label}] to UNIFIED KERNEL (Base64)...`);
            this.csInterface.evalScript(`$._imposition.engine.run("${payloadBase64}")`, (resultRaw) => {
                let result;
                try {
                    result = JSON.parse(window.atob(resultRaw));
                } catch (e) {
                    console.error("Failed to parse engine Structured Return Payload:", e, resultRaw);
    ...
    async handleTrigger(id) {
        ...
        try {
            ...
            console.log("-> Preset found:", preset.label);

            // Phase 8: Data-Driven Rule Compilation
            this._compileRules(preset);

            console.log("-> Orchestrator context:", !!this.preflightOrchestrator, !!this.bridgeInst);

            // --- PREFLIGHT CHECK ---
            if (this.preflightOrchestrator && this.bridgeInst) {
                console.log("🔍 Running Preflight Orchestrator...");
                const isSafe = await this.preflightOrchestrator.runAll({ bridge: this.bridgeInst });
                if (!isSafe) {
                    console.warn("🛑 Preflight checks failed or user cancelled. Halting execution.");
                    return;
                }
            }
            ...
            const engineResult = await this._runImpositionEngineAsync(preset);
            ...
            if (!engineResult || !engineResult.success) {
                ...
                console.error("Imposition Error:", errorMsg);
                alert("Imposition Error:\\n" + errorMsg);
                return;
            }

            console.log("✅ Imposition Execution Successful!", engineResult.data);
