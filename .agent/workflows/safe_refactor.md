...
1. Đọc `.agent/GOVERNANCE.md`
2. Đọc `.agent/skills/HEXAGONAL.md` (nếu có)
...
```
Files to modify:
- [ ] wedding-cep/cep/js/logic/domain/rules.js
...
load("wedding-cep/cep/js/NewName.js"); // Đường dẫn mới
...
> **⚠️ BẮT BUỘC:** Mở đọc và tuân thủ `@[.agent/workflows/testing_guide.md]` để biết cách móc nối Chrome DevTools và chạy E2E Script qua cổng 9097 trước khi đánh dấu tick vào các ô dưới đây.
...
4. **Dead Code Hunting (Săn Rác):** Dùng công cụ phân tích tĩnh hoặc rà bằng tay để quét dự án, tìm và tiêu diệt dứt điểm các file/hàm đã bị mồ côi (orphaned) sau khi đập đi xây lại.
5. **Decoupling Audit (Rà soát Dây điện):** Mở `nx graph` hoặc check thủ công để đảm bảo kiến trúc không bị chọc ngược (Circular Dependency). Các hàm phải tuân thủ luồng tuyến tính.
6. **Cập nhật documentation** nếu có
