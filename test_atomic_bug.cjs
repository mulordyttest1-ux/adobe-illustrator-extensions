...
async function testILST() {
    try {
        ...
        const { result } = await Runtime.evaluate({
            expression: `window.bridge.cs.evalScript(${JSON.stringify(script)}, function(r){ window.TEST_RES = r; })`
        });

        await new Promise(r => setTimeout(r, 1000));

        const { result: actual } = await Runtime.evaluate({ expression: 'window.TEST_RES' });
