...
export const CalendarEngine = {
    ...
    /**
     * Load CSV database from injected content
     * @param {string} csvContent - The raw CSV string
     */
    loadDatabase(csvContent) {
        if (this._isLoaded) return true;
        if (typeof csvContent !== 'string') return false;

        try {
            this._parseCSV(csvContent);
            return true;
        } catch {
            return false;
        }
    ...
    expandDate(date) {
        ...
        if (!this._isLoaded) return result;
