...
Verification Evidence: Devkit tag CI run 29545444980 passed; pinned `npm run devkit:ensure -- --json` passed; `npm run test:machine-setup` passed 19/19; `npm run verify` passed after the EOL review fix; gate check passed; a fresh Windows worktree reported `specify integration status --json` status `ok` with zero missing/modified files and all five managed PowerShell scripts contained zero CRLF sequences.
Product Merge Condition: Product PR CI and the post-install combined workstation doctor must pass before final completion.
